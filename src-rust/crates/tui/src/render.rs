...
use claurst_core::{constants::APP_VERSION, types::Role};
use ratatui::{
    Frame,
    buffer::Buffer,
...
use crate::{
    agents_view::render_agents_menu,
    app::{App, ContextMenuKind, SystemAnnotation, SystemMessageStyle, ToolStatus},
    bypass_permissions_dialog::render_bypass_permissions_dialog,
    context_viz::render_context_viz,
    desktop_upsell_startup::render_desktop_upsell_startup,
    device_auth_dialog::render_device_auth_dialog,
    dialog_select::render_dialog_select,
    dialogs::{render_mcp_approval_dialog, render_permission_dialog},
    diff_viewer::render_diff_dialog,
    elicitation_dialog::render_elicitation_dialog,
    export_dialog::render_export_dialog,
    feedback_survey::render_feedback_survey,
    figures,
    hooks_config_menu::render_hooks_config_menu,
    invalid_config_dialog::render_invalid_config_dialog,
    key_input_dialog::render_key_input_dialog,
    mcp_view::render_mcp_view,
    memory_file_selector::render_memory_file_selector,
    memory_update_notification::render_memory_update_notification,
    messages::{
        RenderContext, render_transcript_assistant_message, render_transcript_assistant_meta,
        render_transcript_live_text, render_transcript_user_message,
    },
    model_picker::render_model_picker,
    notifications::render_notification_banner,
    onboarding_dialog::render_onboarding_dialog,
    overage_upsell::render_overage_upsell,
    overlays::{
        CLAURST_ACCENT, render_global_search, render_help_overlay, render_history_search_overlay,
        render_rewind_flow,
    },
    plugin_views::render_plugin_hints,
    privacy_screen::render_privacy_screen,
    prompt_input::{InputMode, TypeaheadSource, VimMode, input_height, render_prompt_input},
    rustle::rustle_lines,
    session_branching::render_session_branching,
    session_browser::render_session_browser,
    settings_screen::render_settings_screen,
    stats_dialog::render_stats_dialog,
    tasks_overlay::render_tasks_overlay,
    theme_screen::render_theme_screen,
    transcript_turn::{TranscriptTurn, build_transcript_turns},
    virtual_list::{VirtualItem, VirtualList},
    voice_mode_notice::render_voice_mode_notice,
};
...
fn push_rendered_items(
    items: &mut Vec<RenderedLineItem>, lines: Vec<Line<'static>>, message_index: Option<usize>,
...
fn append_turn_items(
    items: &mut Vec<RenderedLineItem>, turn: &TranscriptTurn<'_>, width: u16,
    tool_names: &std::collections::HashMap<String, String>,
    expanded_thinking: &std::collections::HashSet<u64>, frame_count: u64, accent: Color,
) {
    push_rendered_items(
        items,
...
// â”€â”€ System annotation (compact boundary, info notices)
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

fn render_system_annotation_lines(
    lines: &mut Vec<Line<'static>>, ann: &SystemAnnotation, width: usize,
) {
...
fn render_tool_block_lines(
    lines: &mut Vec<Line<'static>>, block: &crate::app::ToolUseBlock, frame_count: u64,
) {
...
fn render_footer(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let right_spans: Vec<Span> = {
        ...
        // 1. Context window usage — show "N% until auto-compact" mirroring TS TokenWarning. When an
        //    update is available and context is below 85%, show the update notification instead to
...
fn render_legacy_history_search(
    frame: &mut Frame, hs: &crate::app::HistorySearch, app: &App, area: Rect,
) {
...
pub fn render_full_status_line(
    data: &StatusLineData, area: Rect, buf: &mut ratatui::buffer::Buffer,
) {
    use ratatui::{
        style::{Color, Modifier, Style},
        text::{Line, Span},
...
pub fn render_agent_progress_line(
    agent_id: &str, status: &str, current_tool: Option<&str>,
) -> Line<'static> {
...
pub fn render_coordinator_status_lines(
    agent_count: usize, completed: usize, active_agents: &[&str],
) -> Vec<Line<'static>> {
