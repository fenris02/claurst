...
fn compute_modal_layout(
    area: Rect, width: u16, height: u16, header_height: u16, footer_height: u16,
) -> ModalLayout {
...
pub fn begin_modal_frame(
    frame: &mut Frame, area: Rect, width: u16, height: u16, header_height: u16, footer_height: u16,
) -> ModalLayout {
...
pub fn begin_modal_buf(
    buf: &mut Buffer, area: Rect, width: u16, height: u16, header_height: u16, footer_height: u16,
) -> ModalLayout {
    let layout = compute_modal_layout(area, width, height, header_height, footer_height);
...
pub fn modal_search_line(
    query: &str, placeholder: &str, placeholder_color: Color, query_color: Color,
) -> Line<'static> {
    if query.is_empty() {
...
pub fn render_history_search_overlay(
    frame: &mut Frame, overlay: &HistorySearchOverlay, history: &[String], area: Rect,
) {
    if !overlay.visible {
        return;
    }
...
fn build_highlighted_spans<'a>(
    text: &str, highlight_positions: &[usize], max_chars: usize, base_style: Style,
    _is_selected: bool,
) -> Vec<Span<'a>> {
...
pub fn render_global_search(
    state: &GlobalSearchState, area: ratatui::layout::Rect, buf: &mut ratatui::buffer::Buffer,
) {
    use std::path::Path;
    ...
    // Build grouped display rows: (is_header, result_idx_or_none, file_label, match_count,
    // result_ref) Group results by file
    #[derive(Clone)]
    enum DisplayRow {
