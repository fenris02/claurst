...
fn motion_find_char(
    text: &str, cursor: usize, target: char, kind: VimFindKind, count: usize,
) -> Option<usize> {
    match kind {
...
fn apply_operator_range(
    op: VimOperator, text: &str, from: usize, to: usize, yank_buf: &mut String, mode: &mut VimMode,
) -> (String, usize) {
...
pub fn apply_vim_key(
    mode: &mut VimMode, text: &mut String, cursor: &mut usize, key: &str, yank_buf: &mut String,
    pending: &mut VimPendingState, last_find: &mut Option<(VimFindKind, char)>,
) -> bool {
...
fn vim_count(
    mode: &mut VimMode, text: &mut String, cursor: &mut usize, key: &str, yank_buf: &mut String,
    pending: &mut VimPendingState, last_find: &mut Option<(VimFindKind, char)>, digits: String,
) -> bool {
...
fn vim_normal(
    mode: &mut VimMode, text: &mut String, cursor: &mut usize, key: &str, yank_buf: &mut String,
    pending: &mut VimPendingState, last_find: &mut Option<(VimFindKind, char)>, count: usize,
) -> bool {
    let n = count.max(1);
    match key {
        // ---- Mode transitions ----
...
fn vim_g(
    text: &mut String, cursor: &mut usize, key: &str, pending: &mut VimPendingState, count: usize,
) -> bool {
    match key {
        "g" => {
...
fn vim_operator(
    mode: &mut VimMode, text: &mut String, cursor: &mut usize, key: &str, yank_buf: &mut String,
    pending: &mut VimPendingState, _last_find: &mut Option<(VimFindKind, char)>, op: VimOperator,
    count: usize,
) -> bool {
...
fn vim_operator_count(
    mode: &mut VimMode, text: &mut String, cursor: &mut usize, key: &str, yank_buf: &mut String,
    pending: &mut VimPendingState, last_find: &mut Option<(VimFindKind, char)>, op: VimOperator,
    count: usize, digits: String,
) -> bool {
    if key.len() == 1 && key.chars().next().unwrap().is_ascii_digit() {
        let new_digits = format!("{}{}", digits, key);
...
pub fn render_prompt_input(
    state: &PromptInputState, area: Rect, buf: &mut Buffer, focused: bool, mode: InputMode,
