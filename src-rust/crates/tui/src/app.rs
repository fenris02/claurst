...
use crate::{
    ...
    context_viz::ContextVizState,
    dialog_select::{DialogSelectState, SelectItem},
    dialogs::{McpApprovalDialogState, PermissionRequest},
    diff_viewer::{DiffPane, DiffViewerState, build_turn_diff},
    export_dialog::{ExportDialogState, ExportFormat},
    mcp_view::{McpServerView, McpToolView, McpViewState, McpViewStatus},
    model_picker::{EffortLevel, ModelPickerState, is_fast_mode_model},
    ...
    plugin_views::PluginHintBanner,
    privacy_screen::PrivacyScreen,
    prompt_input::{InputMode, PromptInputState, VimMode},
    render,
    session_browser::SessionBrowserState,
    settings_screen::SettingsScreen,
    stats_dialog::StatsDialogState,
    tasks_overlay::TasksOverlay,
...
- Lead with the answer or action, not the reasoning";
    ...
    match level {
        "lite" => format!(
            "{}\n\nStrip pleasantries and hedging. Keep full grammar and articles. Just remove the fluff.",
            base
        ),
        "ultra" => format!(
            "{}\n\nAlso drop articles (a/an/the). Compress to short imperative phrases. Numbered steps, no prose between. Absolute minimum words.",
            base
        ),
        _ => format!(
...
impl App {
    ...
    pub fn apply_provider_refresh(
        ...
        auth_store: claurst_core::AuthStore, has_credentials: bool, status_message: String,
    ) {
    ...
    pub fn intercept_slash_command(&mut self, cmd: &str) -> bool {
        ...
        match cmd {
            ...
            "copy" => {
                // Copy last assistant message to clipboard. Attempt arboard; fall back to
                // notification.
                let last = self
                    .messages
                    .iter()
    ...
    pub fn attach_turn_diff_state(
        &mut self, file_history: Arc<parking_lot::Mutex<FileHistory>>,
    ...
    pub fn handle_query_event(&mut self, event: QueryEvent) {
        match event {
            ...
            QueryEvent::TokenWarning { state, pct_used } => {
                // Push a notification for context window warnings (notification + threshold
                // tracking).
