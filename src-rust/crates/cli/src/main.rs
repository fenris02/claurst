...
fn resolve_bridge_config(
    settings: &Settings, auth_credential: &str, use_bearer_auth: bool, is_headless: bool,
...
fn filter_tools_for_agent(
    tools: Arc<Vec<Box<dyn claurst_tools::Tool>>>, access: &str,
) -> Arc<Vec<Box<dyn claurst_tools::Tool>>> {
...
async fn run_headless(
    cli: &Cli, client: Arc<claurst_api::AnthropicClient>,
    tools: Arc<Vec<Box<dyn claurst_tools::Tool>>>, tool_ctx: ToolContext,
    query_config: claurst_query::QueryConfig, cost_tracker: Arc<CostTracker>,
) -> anyhow::Result<()> {
...
async fn run_interactive(
    config: Config, settings: claurst_core::config::Settings,
    client: Arc<claurst_api::AnthropicClient>, tools: Arc<Vec<Box<dyn claurst_tools::Tool>>>,
    tool_ctx: ToolContext, query_config: claurst_query::QueryConfig,
    cost_tracker: Arc<CostTracker>, resume_id: Option<String>,
    bridge_config: Option<claurst_bridge::BridgeConfig>, has_credentials: bool,
    model_registry: Arc<claurst_api::ModelRegistry>,
) -> anyhow::Result<()> {
    use std::time::Duration;

    use claurst_bridge::{BridgeOutbound, TuiBridgeEvent};
    use claurst_commands::{CommandContext, CommandResult, execute_command};
    use claurst_query::{QueryEvent, QueryOutcome};
    use claurst_tui::{
        App, bridge_state::BridgeConnectionState, device_auth_dialog::DeviceAuthEvent,
        notifications::NotificationKind, render::render_app, restore_terminal, setup_terminal,
    };
