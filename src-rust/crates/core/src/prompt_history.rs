...
//   - An advisory lock file (`history.jsonl.lock`) serialises concurrent writers (up to 20 retries
//     × 50 ms back-off).
//   - Large pasted text (> 1 024 bytes) is stored externally in `~/.claurst/pastes/<sha256-hex>`
//     and referenced by hash.
//   - Images are not stored in the JSONL file (they live in the image cache).
//   - `get_history()` reads the file, filters by project, and yields current-session entries first
//     (newest-first within each group).
//   - `expand_pasted_text_refs()` replaces `[Pasted text #N]` / `[Pasted text #N +X lines]`
//     placeholders with the actual text.
//   - `parse_references()` extracts all numeric IDs from reference patterns.
//   - `remove_last_from_history()` undoes the most-recent `add_to_history` call (fast path: pop
//     from pending buffer; slow path: skip-set).
