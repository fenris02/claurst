...
pub use config::{
    AgentDefinition, CommandTemplate, Config, FormatterConfig, McpServerConfig, OutputFormat,
    PermissionMode, ProviderConfig, Settings, SkillsConfig, Theme, default_agents,
...
pub use update_check::{UpdateInfo, check_for_updates};
...
pub use permissions::{
    AutoPermissionHandler, InteractivePermissionHandler, ManagedAutoPermissionHandler,
    ...
    SerializedPermissionRule, format_permission_reason,
};
...
pub mod types {
    ...
    impl Message {
        ...
        pub fn user_local_command_output(
            command: impl Into<String>, output: impl Into<String>,
        ) -> Self {
            Self {
                role: Role::User,
        ...
        pub fn collapsed_read_search(
            tool_name: impl Into<String>, paths: Vec<String>, n_hidden: usize,
        ) -> Self {
            Self {
                role: Role::User,
        ...
        pub fn task_assignment(
            id: impl Into<String>, subject: impl Into<String>, description: impl Into<String>,
        ) -> Self {
            Self {
                role: Role::User,
    ...
    impl Settings {
        ...
        fn merge(base: Self, over: Self) -> Self {
            ...
            fn merge_map<K: std::hash::Hash + Eq + Clone, V: Clone>(
                mut base: HashMap<K, V>, over: HashMap<K, V>,
            ) -> HashMap<K, V> {
                for (k, v) in over {
                    base.insert(k, v);
                }
...
pub mod constants {
    ...
    pub const ANTHROPIC_BETA_HEADER: &str = "interleaved-thinking-2025-05-14,token-efficient-tools-2025-02-19,files-api-2025-04-14,\
         effort-2025-11-24";
...
pub mod permissions {
    ...
    ///   - `tool_name` is `None` (applies to every tool) OR equals the request tool name.
    ///   - `path_pattern` is `None` OR the glob pattern matches the request path.
    #[derive(Debug, Clone, Serialize, Deserialize)]
    ...
    impl PermissionManager {
        ...
        pub fn new(
            mode: crate::config::PermissionMode, settings: &crate::config::Settings,
        ) -> Self {
        ...
        pub fn add_persistent_allow(
            &mut self, tool_name: &str, settings: &mut crate::config::Settings,
        ) -> crate::error::Result<()> {
        ...
        pub fn remove_rule(
            &mut self, idx: usize, settings: &mut crate::config::Settings,
        ) -> crate::error::Result<()> {
...
pub mod history {
    ...
    pub async fn branch_session(
        source_id: &str, at_message_idx: usize, new_title: Option<&str>,
...
pub mod hooks {
    ...
    pub async fn run_hooks(
        hooks: &HashMap<HookEvent, Vec<HookEntry>>, event: HookEvent, ctx: &HookContext,
        working_dir: &Path,
    ) -> HookOutcome {
