...
use claurst_core::{
    ...
    cost::CostTracker,
    error::ClaudeError,
    types::{ContentBlock, Message, ToolResultContent, UsageInfo},
};
...
pub use compact::{
    ...
    TokenWarningState, auto_compact_if_needed, calculate_messages_to_keep_index,
    calculate_token_warning_state, compact_conversation, context_collapse,
    context_window_for_model, format_compact_summary, get_compact_prompt,
    group_messages_for_compact, micro_compact_if_needed, reactive_compact, should_auto_compact,
    should_compact, should_context_collapse, snip_compact,
};
...
pub use skill_prefetch::{
    SharedSkillIndex, SkillDefinition, SkillIndex, format_skill_listing, prefetch_skills,
};
use tokio::sync::mpsc;
...
pub fn fire_post_sampling_hooks(
    _turn_result: &claurst_core::types::Message, config: &claurst_core::config::Config,
) -> PostSamplingHookResult {
    use claurst_core::{config::HookEvent, types::Message};
...
pub fn stop_hooks_with_full_behavior(
    turn_result: &claurst_core::types::Message, config: &claurst_core::config::Config,
...
const MAX_TOKENS_RECOVERY_MSG: &str = "Output token limit hit. Resume directly — no apology, no recap of what \
...
pub async fn run_query_loop(
    client: &claurst_api::AnthropicClient, messages: &mut Vec<Message>, tools: &[Box<dyn Tool>],
    tool_ctx: &ToolContext, config: &QueryConfig, cost_tracker: Arc<CostTracker>,
            ...
            if use_provider_dispatch {
                ...
                if let Some(provider) = provider {
                    ...
                    let hint = match provider_id_str.as_str() {
                        "google" => {
                            "Set GOOGLE_API_KEY or run `claurst auth login --provider google`."
                        }
                        "openai" => {
                            "Set OPENAI_API_KEY or run `claurst auth login --provider openai`."
                        }
                        ...
                        "xai" => "Set XAI_API_KEY.",
                        "github-copilot" => {
                            "Reconnect GitHub Copilot via /connect, or set GITHUB_TOKEN."
                        }
                        "cohere" => "Set COHERE_API_KEY.",
                        _ => {
                            "Set the appropriate API key environment variable or use `claurst auth login`."
                        }
                    };
                    error!(
...
async fn execute_tool(
    name: &str, input: &Value, tools: &[Box<dyn Tool>], ctx: &ToolContext,
) -> ToolResult {
...
fn map_to_anthropic_event(
    ...
    use claurst_api::{
        StreamEvent,
        streaming::{AnthropicStreamEvent, ContentDelta},
    };
...
pub async fn run_single_query(
    client: &claurst_api::AnthropicClient, messages: Vec<Message>, config: &QueryConfig,
