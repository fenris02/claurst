...
fn execute_named_command_from_slash(
    target_name: &str, args: &str, ctx: &CommandContext,
) -> CommandResult {
...
fn export_message_to_markdown(
    msg: &claurst_core::types::Message, all_messages: &[claurst_core::types::Message],
    msg_idx: usize,
) -> String {
...
mod chrome_cdp {
    ...
    async fn cdp_call(
        ws: &mut WebSocketStream<MaybeTlsStream<TcpStream>>, method: &str, params: Value,
    ) -> anyhow::Result<Value> {
        let id = next_id();
...
mod teleport_bundle {
    use claurst_core::{
        permissions::{PermissionAction, SerializedPermissionRule},
        types::Message,
    };
...
impl SlashCommand for TeleportCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        use teleport_bundle::{BUNDLE_VERSION, TeleportBundle, TeleportPermissions};
...
pub fn commands_from_discovered_skills(
    cwd: &std::path::Path, skills_config: &claurst_core::SkillsConfig,
